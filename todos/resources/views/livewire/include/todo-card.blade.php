<div wire:key="{{ $todo->id }}" class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
    <div class="flex justify-between space-x-2">
        ...
        <div class="flex items-center">
            ...
            <div class="">
            <input wire.model="editingtodoname" type="text" class="bg-gray-100  text-gray-900 text-sm rounded block w-full p-2.5" placeholder="Todo..">
                        @error('editingtodoname')
                        <span class="text-red-500 text-xs block">{{ $message }}</span>
                        @enderror
            </div>
            @else
                  <h3 class="text-lg mg-1 text-semibold text-gray-800">{{ $todo->name }}</h3>
            @endif
